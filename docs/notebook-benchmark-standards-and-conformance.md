...
核实日期：2026-09-24。适用于 `feat/benchmark-protocol-correctness` 工作树中的实现；基线提交为 `e022c60`，协议修正尚未提交、合并或推送。本文中的“当前实现”不等于主线或服务器已经部署的版本。
...
1. **冻结运行版本。** 记录本分支最终 commit、产品版本、数据/输入/profile、模型和完整配置；本页当前基线 commit 不能代替尚未提交的源码身份。
