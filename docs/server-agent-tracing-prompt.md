...
```text
请使用包含当前原生评分恢复实现的固定评测提交，保留服务器已有模型配置和修复。当前实现已合并到本地主线；若服务器远程尚未提供该提交，先核对可用的提交身份再执行。阅读 AGENTS.md、docs/native-scoring-recovery.md。此次只更新评测框架，不重打 SN 补丁。
...
## 初始原生验收（历史 prompt，已被上方恢复步骤替代；不要按本节重新安排任务）
