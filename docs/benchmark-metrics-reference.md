...
| IFEval | Native（N） | 全部指令满足<br>`deepeval.ifeval.all_instructions.v1` | 确定性 / DeepEval IFEval verifier。直接调用固定版本 SDK verify_instruction_compliance，按 IFEval.predict 的逻辑合取结果；保存每个位置的指令、参数、结果和理由，无人工正反例审计前置条件。 所有指令均通过 → 1；任意指令失败 → 0。有效题级分数取均值，并单列评分覆盖率。 限制：使用 DeepEval 4.2.2 的实际规则（包括默认分支），不是原论文 strict/loose 复现。SN 正文的引用与解释原样参与检查；执行异常和未正常回答单独记录。 |
| IFEval | SN Product（R；chunk / reasoning） | 全部指令满足<br>`product.ifeval.all_instructions.full_body.v2` | 确定性 / DeepEval IFEval verifier。直接调用固定版本 SDK verify_instruction_compliance，按 IFEval.predict 的逻辑合取结果；保存每个位置的指令、参数、结果和理由，无人工正反例审计前置条件。 所有指令均通过 → 1；任意指令失败 → 0。有效题级分数取均值，并单列评分覆盖率。 限制：使用 DeepEval 4.2.2 的实际规则（包括默认分支），不是原论文 strict/loose 复现。SN 正文的引用与解释原样参与检查；执行异常和未正常回答单独记录。 |
