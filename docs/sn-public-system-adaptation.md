...
每 suite/mode 新建隔离运行时与 notebook，使用固定资料并集和全库范围，不逐题限制 gold source。每题不传 conversation_id，禁用 Memory/偏好/经验注入。资料去重且超过 40 个时显式报错，要求未来准备较小的冻结选集，不静默删题或减少分母。不得为通过无来源检查导入无关占位资料。
...
本轮完成上述七套新系统适配，原三套产品路径保留；所有新功能未运行验证。下一步是允许执行后做离线回归，再冻结小样本和小规模在线验证。生产代码/配置、旧 baseline、weekly timer、KG、重排、PDF/OCR、交互可靠性和资料更新一致性保持原边界。
