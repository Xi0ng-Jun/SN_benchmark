...
| 共同题配对图 | 同资料同配置下两种模式在共同题上差多少 | 只用双方都有有效分数的配对，另报排除数 |
...
3. 查看各组有效评分均值和覆盖，再看共同有效题的配对均值、差值以及提升/持平/下降数量。
...
当前严格配对主要支持 **SN chunk ↔ reasoning**。Native 与 Product、不同模型版本或不同资料配置可以分面查看各自成绩，但不能直接获得配对提升结论。Task/状态筛选导致两组题目不同时，配对仅计算共同有效题，显示仅在单侧的题、单侧缺分和双方缺分；不做显著性检验或因果解释。
